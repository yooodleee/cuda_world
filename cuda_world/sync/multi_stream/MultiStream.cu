...
void main(void) {
	...
	int chunckSize = ARRAY_SIZE / NUM_STREAMS;

	timer.onTimer(4);
	for (int i = 0; i < NUM_STREAMS; i++) {
		int offset = chunckSize * i;
		cudaMemcpyAsync(dIn + offset, in + offset, sizeof(int) * chunckSize, cudaMemcpyHostToDevice, stream[i]);
	}
	...
	for (int i = 0; i < NUM_STREAMS; i++) {
		int offset = chunckSize * i;
		cudaMemcpyAsync(out2 + offset, dOut + offset, sizeof(int) * chunckSize, cudaMemcpyDeviceToHost, stream[i]);
	}
