...
#define NUM_DATA 1024		/* The size of the vector */

__global__ void vecAdd(int* _a, int* _b, int* _c) {		/* Simple vector sum kernel (Max vector size: 1024) */
	int tID = threadIdx.x;
...
int main(void) {
	...
	int memSize = sizzeof(int) * NUM_DATA;
	printf("%d element, memSize = %d bytes\n", NUM_DATA, memSize);
	...
	for (int i = 0; i < NUM_DATA; i++) {		/* Data generation */
		a[i] = rand() % 10;
		b[i] = rand() % 10;
	}

	for (int i = 0; i < NUM_DATA; i++)			/* Vectors sum on host (for performance comparison) */
		hc[i] = a[i] + b[i];
